...
//        });
    ...
    topicNodeEnter.selectAll("text.topiclabel").on("click", function(d) {
        ...
        if (d.clickstate == 1) {
            ...
            undoHighlightOntologyNodesGivenTopic(d.id-1);
        }
        else
            d.clickstate = 1;
        topicName = d.name;
...
//        console.log('yes');
        ...
        d3.selectAll("g.ontology.node").on("mouseover", mouseoverOntologyNode);
        ...
        d3.selectAll("g.ontology.node").on("mouseout", mouseoutOntologyNode);
...
function highlightOntologyNodesGivenTopic(topic_id) {
    let keys = getKeyByValue(ontology_map, topic_id);
...
function undoHighlightOntologyNodesGivenTopic(topic_id) {
    let keys = getKeyByValue(ontology_map, topic_id);
    keys.forEach(function (key) {
        d3.select("#"+key)
            .attr("fill", "green")
...
//        }
    ...
    topicName = d.name;
    highlightTopicsTreeNodesTopic(d);
    highlightOntologyNodesGivenTopic(d.id-1);
...
function mouseoverOntologyNode(d) {
    if (dragStarted) {
        selectedNode = d;
//        if (draggingNode.name != d.name) {
//            nodesTopicTree.sort(function(a, b) { // select the parent and sort the path's
...
//        }
    ...
    var backgroundcolor = "rgb(228,236,241)";
    d3.select("rect#"+d["abbr"])
        .attr("fill", "orange")
        .style("opacity", 0.5);

    for (i = 0; i < ontology_map[d["abbr"]].length; i++) {
        highlightTopicsTreeNodesTopic(nodesTopicTree[ontology_map[d["abbr"]][i]]);
    }

}

function mouseoutOntologyNode(d) {
    ...
    d3.select("rect#"+d["abbr"])
        ...
        for (i = 0; i < ontology_map[d["abbr"]].length; i++) {
            undoHighlightTopicsTreeNodesTopic(nodesTopicTree[ontology_map[d["abbr"]][i]]);
        }
