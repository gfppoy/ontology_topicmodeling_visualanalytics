...
const svg = d3.select('body').append('svg')
            .attr("width", 1000)
            .attr("height", 1000)
            .attr("transform", "translate(-630,-950)");
...
let ontology_map = null;
let ontology_names = null;

jQuery.ajaxSetup({async:false});

$.getJSON("data/" + dataset + "/" + "ontology_map.json", function(obj) {
    ontology_map = obj;
});

$.getJSON("data/" + dataset + "/" + "ontology_names.json", function(obj){
    ontology_names = obj;
});
...
.attr("transform", (_, i)=>translateString(ONTO_X, ONTO_Y + i*ONTO_HEIGHT)))[0];

count = 0;

g_onto_node.forEach((d, i, nodes)=> {
    let g_this = d3.select(nodes[i]);

    g_this.append("rect")
    .attr('id', d.__data__["abbr"])
    ...
    let text = g_this.append("text")
        ...
        .text(d.__data__["name"]);

    if (text[0][0].__data__["name"] > LABEL_WIDTH) {
...
const g_graph = parent.append("g")
.attr("class", "graph");

g_graph_links = g_graph.append("g")
                    .attr("class", "links");

Object.keys(ontology_map).forEach((concept, i) => {
    ontology_map[concept].forEach((topic => {
        g_graph_links.append("line")
            .attr("stroke", "black")
            .attr("stroke-width", 3)
            .attr("opacity", 0.7)
            .attr("stroke-linecap", "round")
            .attr("x1", 770)
            .attr("y1", ONTO_Y+i*ONTO_HEIGHT)
            .attr("x2", 835)
            .attr("y2", 15+topic*30);
    }));
});
...
$.getJSON("data/" + dataset + "/" + "topicHierarchy_" + dataset + ".json", function (json) {
...
function undohighlightTopicLinks() {
    ...
    //alert(filename);
    undoHighlightTopicsbyConversation(currentConversationid);
